...
function Index() {
  ...
  return <main>
    ...
    <section className="section process-section"><div className="shell"><header className="center-heading"><span className="kicker">A proven learning process</span><h2>From Beginner to Job-Ready</h2></header><div className="process-grid">{([[BookOpen,"01","Learn","Build strong technical foundations."],[Laptop,"02","Practice","Work with hands-on security labs."],[Code2,"03","Build","Complete projects and real-world scenarios."],[BriefcaseBusiness,"04","Prepare","Resume, interview and career preparation."]] as const).map(([Icon,n,t,d])=><div key={t}><span className="process-icon"><Icon/></span><small>{n}</small><h3>{t}</h3><p>{d}</p></div>)}</div></div></section>

    <section className="section audience"><div className="shell"><header className="center-heading"><span className="kicker">Designed for every starting point</span><h2>Who Can Join?</h2></header><div className="audience-grid">{([[GraduationCap,"Students","Start your cybersecurity journey from the fundamentals."],[UserRound,"Fresh Graduates","Build practical skills beyond your academic curriculum."],[Laptop,"IT Professionals","Upgrade your existing IT and networking skills."],[UsersRound,"Career Switchers","Transition into cybersecurity with structured hands-on training."]] as const).map(([Icon,t,d])=><article key={t}><Icon/><h3>{t}</h3><p>{d}</p></article>)}</div></div></section>
